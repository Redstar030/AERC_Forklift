...
void Controller_Update(void)
{
    ...
    int16_t wz =  scale_stick(controller_state.r_stick_y); 
    Mecanum_Drive(vx, vy, wz);

    if      (controller_state.r1) Fork1_Up();
    else if (controller_state.r2) Fork1_Down();
    else                          Fork1_Stop();

    if      (controller_state.l1) Fork2_Up();
    else if (controller_state.l2) Fork2_Down();
    else                          Fork2_Stop();
}